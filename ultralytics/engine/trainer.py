...
class Distillation_loss:
    def __init__(self, model_s, model_t, layers, distiller="CWDLoss"):  # model must be de-paralleled
        ...
        channels_s = self.get_channels(model_s, layers)
        channels_t = self.get_channels(model_t, layers)
        ...
        for mname, ml in model_t.named_modules():
    ...
    def get_channels(self, model, layers):
        channels = []
        for idx in map(int, layers):
            n = model.model[idx].cv2.conv.out_channels
            channels.append(n)
        return channels
...
class BaseTrainer:
    ...
    def __init__(self, cfg=DEFAULT_CFG, overrides=None, _callbacks=None):
        ...
        """
        self.exist_dis = False
        if "Distillation" in overrides:
            self.Distillation = overrides["Distillation"]
            self.loss_type = overrides["loss_type"]
            self.layers = overrides["layers"]
            del overrides["Distillation"]
            del overrides["loss_type"]
            del overrides["layers"]
            self.exist_dis = True
